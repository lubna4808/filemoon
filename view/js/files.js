...
const shareFile = async (id, e) => {
    ...
    try {
        ...
        sendButton.innerHTML = `<i class="fa fa-spinner fa-spin mr-2"></i> wait`
...
const uploadFile = async (e) => {
    try {
        ...
        const { data } = await axios.post('/api/file', formdata, getToken())
...
/* ✅ FIXED fetchFiles (BRACKET ISSUE SOLVED) */
...
const openModelForShare = (id, filename) => {
    new Swal({
        ...
        <form class="text-left flex flex-col gap-6" onsubmit="shareFile('${id}',event)">
            ...
            <button id="send-button" class="bg-indigo-400 hover:bg-indigo-500 text-white rounded py-3 px-8 w-fit font-medium">
                Send
            </button>
